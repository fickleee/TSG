...
class TSGDataModule(pl.LightningDataModule):
    ...
    '''
    def __init__(self, data_path_dict, window=96, val_portion=0.1, as_tensor:bool=True, normalize="centered_pit", batch_size=128, num_workers=0, pin_memory=True, drop_last=False, reweight=False, input_channels=1, num_variables_dict=None, **kwargs):
        super().__init__()
        ...
        # 记录每个数据集的变量数（用于GNN）
        # num_variables_dict: {data_name: num_variables}
        self.num_variables_dict = num_variables_dict or {}
        self.original_channels_dict = {}  # 记录原始通道数
    ...
    def prepare_data(self) -> None:
        ...
        for data_name, data_path in self.data_path_dict.items():
            ...
            # 记录原始通道数
            self.original_channels_dict[data_name] = original_channels
            
            # 如果num_variables_dict中没有指定，使用原始通道数
            ...
            print(f"Original channels: {original_channels}, Variables for GNN: {self.num_variables_dict[data_name]}")
    ...
    def train_dataloader(self):
        ...
        # 将变量数信息传递给dataset
        train_dataset.num_variables_dict = self.num_variables_dict
        sampler = None
        if self.reweight:
    ...
    def val_dataloader(self):
        ...
        # 将变量数信息传递给dataset
        val_dataset.num_variables_dict = self.num_variables_dict

...
class LatentDiffusion(DDPM):
    ...
    def get_learned_conditioning(self, c, return_mask=False, data_key=None, num_variables_dict=None):
        if self.cond_stage_forward is None:
            if hasattr(self.cond_stage_model, 'encode') and callable(self.cond_stage_model.encode):
                ...
                # 如果cond_stage_model支持data_key和num_variables_dict，传递这些参数
                if hasattr(self.cond_stage_model, 'forward') and 'data_key' in self.cond_stage_model.forward.__code__.co_varnames:
                    c, mask = self.cond_stage_model(c, data_key=data_key, num_variables_dict=num_variables_dict)
                else:
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        if self.model.conditioning_key is not None:
            ...
            if not self.cond_stage_trainable or force_c_encode:
                # 获取data_key和num_variables_dict（如果batch中有）
                batch_data_key = batch.get('data_key', None)
                batch_num_variables_dict = getattr(self, 'num_variables_dict', None)
                
                # 如果x被切片了，data_key也需要相应切片
                if bs is not None and batch_data_key is not None:
                    if isinstance(batch_data_key, torch.Tensor):
                        batch_data_key = batch_data_key[:bs].to(self.device)
                    else:
                        batch_data_key = batch_data_key[:bs]
                
                if isinstance(xc, dict) or isinstance(xc, list):
                    # import pudb; pudb.set_trace()
                    c, mask = self.get_learned_conditioning(xc, return_mask=True, 
                                                          data_key=batch_data_key, 
                                                          num_variables_dict=batch_num_variables_dict)
                else:
    ...
    def forward(self, x, c, *args, **kwargs):
        ...
        if self.model.conditioning_key is not None:
            ...
            if self.cond_stage_trainable:
                # 从kwargs中获取data_key和num_variables_dict
                data_key = kwargs.get('data_key', None)
                num_variables_dict = getattr(self, 'num_variables_dict', None)
                c = self.get_learned_conditioning(c, return_mask=True, 
                                                data_key=data_key, 
                                                num_variables_dict=num_variables_dict)

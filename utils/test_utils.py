...
def test_model_unseen(model, data, trainer, opt, logdir):
    all_metrics = {}
    seq_len = opt.seq_len
    zero_shot_data_dir = Path(f'{data_root}/ts_data/new_zero_shot_data')
    
    # 检查零样本数据目录是否存在
    if not zero_shot_data_dir.exists():
        print(f"警告: 零样本测试数据目录不存在: {zero_shot_data_dir}")
        print("跳过零样本测试。如需测试零样本性能，请先运行数据准备脚本生成测试数据。")
        print("提示: 可以运行 utils/prepare_datasets.py 来生成零样本测试数据。")
        return
    
    for data_name in ['stock', 'web']:
        test_file = zero_shot_data_dir / f'{data_name}_{seq_len}_test_sample.npy'
        
        # 检查测试数据文件是否存在
        if not test_file.exists():
            print(f"警告: 零样本测试数据文件不存在: {test_file}")
            print(f"跳过 {data_name} 数据集的零样本测试。")
            continue
        
        try:
            data_result_dicts = []
            uni_ori_data = np.load(str(test_file))
            if data_name == 'web':
            ...
            total_samples = 2000
            for k in [3, 10, 100]: 
                k_file = zero_shot_data_dir / f'{data_name}_{seq_len}_k_{k}_sample.npy'
                if not k_file.exists():
                    print(f"警告: k={k} 的提示数据文件不存在: {k_file}，跳过该k值。")
                    continue
                    
                k_samples = np.load(str(k_file))
                for i in range(1):
    ...
    if all_metrics:
        ...
        print("警告: 没有成功完成任何零样本测试。所有测试数据文件可能都不存在。")
        print("提示: 可以运行 utils/prepare_datasets.py 来生成零样本测试数据。")
